...
func TestSeedDanceApifoxCollectionContract(t *testing.T) {
	const collectionName = "seed-dance-apifox.postman_collection.json"
	collection := loadJSONDocument(t, collectionName)

	assert.Equal(
		t,
		"https://schema.getpostman.com/json/collection/v2.1.0/collection.json",
		stringAt(t, collection, "info", "schema"),
	)
	assert.Equal(t, "New API - 无审核 Seed Dance", stringAt(t, collection, "info", "name"))

	variables := map[string]string{}
	for _, rawVariable := range sliceAt(t, collection, "variable") {
		variable := objectValue(t, rawVariable)
		variables[stringAt(t, variable, "key")] = stringAt(t, variable, "value")
	}
	assert.Equal(t, "http://TARGET_HOST", variables["base_url"])
	assert.Equal(t, "NEW_API_TOKEN", variables["api_key"])
	assert.Equal(t, "TASK_ID", variables["task_id"])
	assert.Equal(t, "seedance-uncensored", variables["model"])
	assert.Equal(t, "IMAGE_BASE64", variables["image_base64"])
	assert.Equal(t, "/ABSOLUTE/PATH/TO/INPUT.png", variables["input_image_path"])

	auth := objectAt(t, collection, "auth")
	assert.Equal(t, "bearer", stringAt(t, auth, "type"))
	bearer := sliceAt(t, auth, "bearer")
	require.Len(t, bearer, 1)
	assert.Equal(t, "token", stringAt(t, objectValue(t, bearer[0]), "key"))
	assert.Equal(t, "{{api_key}}", stringAt(t, objectValue(t, bearer[0]), "value"))

	items := map[string]map[string]any{}
	requests := map[string]map[string]any{}
	var walkItems func([]any)
	walkItems = func(rawItems []any) {
		for _, rawItem := range rawItems {
			item := objectValue(t, rawItem)
			if nested, ok := item["item"].([]any); ok {
				walkItems(nested)
				continue
			}
			name := stringAt(t, item, "name")
			_, duplicate := requests[name]
			require.False(t, duplicate, "duplicate Apifox request name %q", name)
			request, ok := item["request"].(map[string]any)
			require.True(t, ok, "leaf collection item %q must contain a request", name)
			items[name] = item
			requests[name] = request
		}
	}
	walkItems(sliceAt(t, collection, "item"))

	expectedRequests := map[string]struct {
		method string
		url    string
	}{
		"服务状态":                     {method: "GET", url: "{{base_url}}/api/status"},
		"创建 T2V - 全参数 JSON":        {method: "POST", url: "{{base_url}}/v1/videos"},
		"创建 I2V - JSON Base64":     {method: "POST", url: "{{base_url}}/v1/videos"},
		"创建 I2V - multipart 文件":    {method: "POST", url: "{{base_url}}/v1/videos"},
		"查询任务状态":                   {method: "GET", url: "{{base_url}}/v1/videos/{{task_id}}"},
		"下载完成视频":                   {method: "GET", url: "{{base_url}}/v1/videos/{{task_id}}/content"},
		"拒绝 duration=0":            {method: "POST", url: "{{base_url}}/v1/videos"},
		"拒绝冲突的 duration 与 seconds": {method: "POST", url: "{{base_url}}/v1/videos"},
		"拒绝无效分辨率":                  {method: "POST", url: "{{base_url}}/v1/videos"},
		"拒绝 T2V 480P":              {method: "POST", url: "{{base_url}}/v1/videos"},
		"拒绝字符串布尔值":                 {method: "POST", url: "{{base_url}}/v1/videos"},
		"拒绝无效 Base64 图片":           {method: "POST", url: "{{base_url}}/v1/videos"},
		"查询不存在的任务":                 {method: "GET", url: "{{base_url}}/v1/videos/TASK_DOES_NOT_EXIST"},
	}
	assert.Len(t, requests, len(expectedRequests))
	for name, expected := range expectedRequests {
		request, ok := requests[name]
		require.True(t, ok, "missing Apifox request %q", name)
		assert.Equal(t, expected.method, stringAt(t, request, "method"), name)
		assert.Equal(t, expected.url, stringAt(t, request, "url", "raw"), name)
	}

	assert.Equal(
		t,
		"raw",
		stringAt(t, requests["创建 T2V - 全参数 JSON"], "body", "mode"),
	)
	assert.Equal(
		t,
		"formdata",
		stringAt(t, requests["创建 I2V - multipart 文件"], "body", "mode"),
	)
	for _, rawHeader := range sliceAt(t, requests["创建 I2V - multipart 文件"], "header") {
		header := objectValue(t, rawHeader)
		assert.False(
			t,
			strings.EqualFold(stringAt(t, header, "key"), "Content-Type"),
			"multipart Content-Type must be generated by Apifox with a boundary",
		)
	}

	jsonI2VBody := stringAt(t, requests["创建 I2V - JSON Base64"], "body", "raw")
	assert.Contains(t, jsonI2VBody, `"input_reference": "{{image_base64}}"`)
	assert.NotContains(t, jsonI2VBody, "data:image/png;base64,")

	testScript := func(name string) string {
		t.Helper()
		for _, rawEvent := range sliceAt(t, items[name], "event") {
			event := objectValue(t, rawEvent)
			if stringAt(t, event, "listen") != "test" {
				continue
			}
			script := strings.Join(stringSlice(t, event, "script", "exec"), "\n")
			require.NotEmpty(t, script, "%s test script must not be empty", name)
			return script
		}
		require.FailNow(t, "missing test script", name)
		return ""
	}
	...
	for _, test := range []struct {
		...
		{name: "创建 T2V - 全参数 JSON", expectedSize: "720P"},
		{name: "创建 I2V - JSON Base64", expectedSize: "480P"},
		{name: "创建 I2V - multipart 文件", expectedSize: "480P"},
	} {
		script := testScript(test.name)
		assert.Contains(t, script, `pm.expect(body.id).to.be.a("string")`)
		assert.Contains(t, script, `pm.expect(body.task_id).to.eql(body.id)`)
		assert.Contains(t, script, `pm.expect(body.model).to.eql("seedance-uncensored")`)
		assert.Contains(t, script, `pm.expect(Number.isInteger(body.progress)).to.eql(true)`)
		assert.Contains(t, script, `pm.expect(Number.isInteger(body.created_at)).to.eql(true)`)
		assert.Contains(
			t,
			script,
			`pm.expect(body.seconds).to.eql(String(pm.variables.get("duration")))`,
		)
		assert.Contains(t, script, fmt.Sprintf(`pm.expect(body.size).to.eql(%q)`, test.expectedSize))
		assert.Contains(t, script, `pm.environment.set("task_id", body.id)`)
		assert.Contains(t, script, `pm.collectionVariables.set("expected_seconds", body.seconds)`)
		assert.Contains(t, script, `pm.collectionVariables.set("expected_size", body.size)`)
	}

	statusScript := testScript("查询任务状态")
	assert.Contains(t, statusScript, `pm.expect(body.id).to.eql(expectedTaskId)`)
	assert.Contains(t, statusScript, `pm.expect(body.task_id).to.eql(expectedTaskId)`)
	assert.Contains(
		t,
		statusScript,
	...
	assert.Contains(t, statusScript, `pm.expect(body).not.to.have.property("completed_at")`)

	downloadScript := testScript("下载完成视频")
	assert.Contains(t, downloadScript, `video/mp4`)
	assert.Contains(t, downloadScript, `Content-Disposition`)
	assert.Contains(t, downloadScript, `X-Content-Type-Options`)

	data, err := os.ReadFile(collectionName)
	require.NoError(t, err)
	text := string(data)
	assert.NotRegexp(t, regexp.MustCompile(`\bsk-[A-Za-z0-9_-]{16,}\b`), text)
	assert.NotRegexp(t, regexp.MustCompile(`\b(?:\d{1,3}\.){3}\d{1,3}\b`), text)
	assert.NotRegexp(
		t,
		regexp.MustCompile(`(?i)\bBearer\s+(?:sk-[A-Za-z0-9_-]+|[A-Za-z0-9_-]{16,})\b`),
		text,
	)
}
