...
func TestTokenPreconsumeRetryRepairsStaleCacheAfterInvalidationFailure(t *testing.T) {
	...
	require.NoError(t, cacheSetTokenForTest(before))
	...
	assert.True(t, server.Exists(getTokenCacheFenceKey(before.Key)))
	reloaded, err := GetTokenByKey(before.Key, false)
	require.NoError(t, err)
	assert.Equal(t, 900, reloaded.RemainQuota)
	assert.False(t, server.Exists(cacheKey), "the mutation fence must block stale cache refill")
	server.FastForward(time.Duration(tokenCacheFenceSeconds+1) * time.Second)
	_, err = GetTokenByKey(before.Key, false)
	require.NoError(t, err)
...
func TestTokenRefundRetryRepairsStaleCacheAfterInvalidationFailure(t *testing.T) {
	...
	server.FastForward(time.Duration(tokenCacheFenceSeconds+1) * time.Second)
	require.NoError(t, cacheSetTokenForTest(consumed))
	...
	require.NoError(t, err)
	assert.False(t, server.Exists(cacheKey))
	assert.True(t, server.Exists(getTokenCacheFenceKey(consumed.Key)))
	reloaded, err := GetTokenByKey(consumed.Key, false)
	require.NoError(t, err)
	assert.Equal(t, 1_000, reloaded.RemainQuota)
	assert.False(t, server.Exists(cacheKey), "the mutation fence must block stale cache refill")
	server.FastForward(time.Duration(tokenCacheFenceSeconds+1) * time.Second)
	_, err = GetTokenByKey(consumed.Key, false)
	require.NoError(t, err)
